...
-spec equal(FTY, FTY) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(FTZ, FTZ) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, FUA} | {error, any()}) -> FUA.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, FUF}) -> FUF.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(FUI)) -> FUI.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
